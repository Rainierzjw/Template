...
namespace Misc
{
    ...
    public class CameraCtrl : MonoSingleton<CameraCtrl>
    {
        ...
        //[Button]
        public void MoveToTarget(Transform tnf, float time, Action action = null)
        {
            if (seq != null) seq.Kill();
            seq = DOTween.Sequence();
            var pos = new Vector3(tnf.position.x, transform.position.y, tnf.position.z); //transform.y
            seq.Append(transform.DOMove(pos, time));
            seq.Join(h_tnf.DOMoveY(tnf.position.y, time));
            seq.Join(h_tnf.DOLocalRotate(new Vector3(0, tnf.eulerAngles.y, 0), time));
            seq.Join(v_tnf.DOLocalRotate(new Vector3(tnf.eulerAngles.x, 0, 0), time));
