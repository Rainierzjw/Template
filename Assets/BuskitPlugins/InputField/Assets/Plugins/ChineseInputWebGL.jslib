var ChineseInputWebGL = {
	...
	InputShow: function(GameObjectName_,inputID_,v_,fontsizeT_,indexStr_,inputRectStr_,isMultiNewLine_,isReadOnly_)
	{
		...
		var isMultiNewLine = isMultiNewLine_;
		...
		document.onkeydown = function (event) {
			...
			if (event.keyCode == 65 && event.ctrlKey) {//捕捉Ctrl+A事件
				...
				//回车不换行
					...
					console.log("回车不换行");
					var textAre= document.getElementById("ChineseInputWebGLId");
				    textAre.blur();
